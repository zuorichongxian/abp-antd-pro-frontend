...
class OrganizationUnitsList extends Component {
  ...
  getNodeTreeRightClickMenu() {
    ...
    const menu = (
      <Menu onClick={this.handleMenuClick} style={tmpStyle} subMenuCloseDelay={0.1}>
        <Menu.Item key="add">
          <PlusCircleOutlined />
          {'添加子组织'}
        </Menu.Item>
  ...
  render() {
    const loop = (data) =>
      data.map((item) => {
        ...
        return (
          <TreeNode key={item.key} title={item.displayName} displayName={item.data.displayName} />
        );
      });
        ...
        render: (value, row, index) => {
          return (
            <Button
              type="danger"
              shape="circle"
              onClick={() => this.removeUser(value.id)}
              icon={<CloseOutlined />}
            />
          );
        },
        ...
        <Row gutter={16}>
          ...
          <Col span={16}>
            <Card
              ...
              extra={
                userCardTitle == undefined ? (
                  ''
                ) : (
                  <Button type="primary" onClick={() => this.addMemberModalVisible(true)}>
                    添加用户
                  </Button>
                )
              }
              ...
              {userCardTitle == undefined ? (
                ...
                <Table
                  ...
                  pagination={{
                    pageSize: 10,
                    total:
                      organizationUnitUsersData == undefined
                        ? 0
                        : organizationUnitUsersData.totalCount,
                    defaultCurrent: 1,
                  }}
                  loading={organizationUnitUsersData == undefined ? true : false}
                  dataSource={
                    organizationUnitUsersData == undefined ? [] : organizationUnitUsersData.items
                  }
