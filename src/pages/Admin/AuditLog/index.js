...
const dateRanges = [
  moment().subtract(7, 'day').format('YYYY-MM-D h:mm:ss.sssssss'),
  moment().format('YYYY-MM-D h:mm:ss.sssssss'),
];
          ...
          <Col md={8} sm={24}>
            <FormItem label="日期范围">
              {getFieldDecorator('DateRange', {
                initialValue: [
                  moment(dateRanges[0], 'YYYY/MM/DD'),
                  moment(dateRanges[1], 'YYYY/MM/DD'),
                ],
              })(<RangePicker />)}
            </FormItem>
          </Col>
  ...
  renderAdvancedForm() {
    ...
    return (
      <Form onSubmit={this.handleSearch} layout="inline">
        <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
          <Col md={8} sm={24}>
            <FormItem label="日期范围">{getFieldDecorator('DateRange')(<RangePicker />)}</FormItem>
          </Col>
          <Col md={8} sm={24}>
            <FormItem label="用户名">
              {getFieldDecorator('UserName')(<Input placeholder="请输入" />)}
          ...
          <div>
            ...
            <Row>
              <Col span={12}>
                <DescriptionItem
                  title="时间"
                  content={moment(auditLogViews.serviceName).format('YYYY年MM月D日 h:mm:ss')}
                />
              </Col>
              <Col span={12}>
                <DescriptionItem
                  title="持续时间"
                  content={auditLogViews.executionDuration + 'ms'}
                />
              </Col>
            </Row>
            <Row>
    ...
    const columns = [
      ...
      },
      {
        ...
        render: (value, row, index) => {
          if (value) return <CloseCircleTwoTone twoToneColor="#F5222D" />;
          else return <CheckCircleTwoTone twoToneColor="#1890FF" />;
        },
      },
      {
        title: '时间',
      ...
      { title: 'IP地址', dataIndex: 'clientIpAddress', key: 'clientIpAddress' },
      {
        ...
        key: 'browserInfo',
        render: (value, row, index) => {
          return <Tooltip title={value}>{truncateStringWithPostfix(value, 20)}</Tooltip>;
        },
      },
    ];
